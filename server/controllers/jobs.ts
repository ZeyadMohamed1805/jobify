...
const getJob = async ( request: Request, response: Response ) => {
    ...
    if ( !job ) return response.status( 404 ).json( { success: false, message: "No Job Was Found With The Requested ID." } );
    return response.status( 200 ).json( { success: true, message: "Job Request Is Successful", job: job } );
}

// POST Job
const postJob = async ( request: Request, response: Response ) => {
    const { company, position } = request.body;
    if ( !company || !position ) return response.status( 400 ).json( { success: false, message: "Request Is Missing Company Or Position" } );
...
// PATCH Job
const patchJob = async ( request: Request, response: Response ) => {
    const { company, position } = request.body;
...
// DELETE Job
const deleteJob = async ( request: Request, response: Response ) => {
    const { id } = request.params;
